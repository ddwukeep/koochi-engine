...
export class LandingArea extends Area
{
    ...
    setLetters()
    {
        ...
        // Brand preview: keep the original physical letter colliders, but replace
        // their visible meshes with one locally generated 3D wordmark.
        for(const reference of references)
        ...
        const geometry = new TextGeometry('KOOCHI ENGINE', {
            font,
            size: 1.45,
            depth: 0.46,
            curveSegments: 5,
        ...
        const targetWidth = firstReference.position.distanceTo(lastReference.position) + 1.4
        geometry.scale(targetWidth / width, 1, 1)
        geometry.computeBoundingBox()
        geometry.translate(
        ...
        const wordmarkMaterial = new MeshDefaultMaterial({
            colorNode: color(0x6F6A87),
            hasWater: false,
        })
        const wordmark = new THREE.Mesh(geometry, wordmarkMaterial)
        wordmark.name = 'koochiEngineWordmark'
        wordmark.position.copy(firstReference.position).add(lastReference.position).multiplyScalar(0.5)
        wordmark.quaternion.copy(firstReference.quaternion)
        wordmark.castShadow = true
        wordmark.receiveShadow = true
        this.game.scene.add(wordmark)

        this.objects.hideable.push(wordmark)
    }
